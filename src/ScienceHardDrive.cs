...
namespace ScienceHardDrives {
	...
	public class ScienceHardDrive : PartModule, IScienceDataContainer {
		...
		private HardDriveManager hardDriveManager = null;
		...
		public override void OnStart(StartState state) {
			...
			hardDriveManager = HardDriveManager.instance;
			...
			if(!xfering) {
				...
				while((kvp = hardDriveManager.getNextInQueue(this)).Key != null) {
					...
					if(data.dataAmount <= capacity - usedSpace) {
						if(container is ScienceHardDrive && ((ScienceHardDrive)container).xferRate < xferRate) {
							transferRate = ((ScienceHardDrive)container).xferRate;
						}
