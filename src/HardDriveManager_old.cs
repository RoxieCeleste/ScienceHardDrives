...
namespace ScienceHardDrives {
	...
	public class QueuePacket_old {
		...
		public ScienceHardDrive_old drive {
			get {
				return _drive;
			}
		}

		public QueuePacket_old(ScienceHardDrive_old drive, IScienceDataContainer container, ScienceData data) {
	...
	public class HardDriveManager_old : MonoBehaviour {

		public static HardDriveManager_old instance;
		...
		private ScienceHardDrive_old[] drives;
		private Part[] containers;

		private ScienceHardDrive_old _selectedDrive;
		private Part _selectedContainer;

		private bool xferNonrerunnable = false;
		//private QueuePacket queuePacket = null;

		private Dictionary<ScienceHardDrive_old, Dictionary<ScienceData, IScienceDataContainer>> queues = new Dictionary<ScienceHardDrive_old, Dictionary<ScienceData, IScienceDataContainer>>();
		...
		private ScienceHardDrive_old selectedDrive {
			get {
		...
		private void OnWindow(int indowId) {
			...
			foreach(ScienceHardDrive_old d in drives) {
		...
		private void queueData(ScienceHardDrive_old drive, IScienceDataContainer container, ScienceData data) {
		...
		public KeyValuePair<ScienceData, IScienceDataContainer> getNextInQueue(ScienceHardDrive_old drive) {
			Dictionary<ScienceData, IScienceDataContainer> queue;
		...
		public void removeFromQueue(ScienceHardDrive_old drive, ScienceData data) {
			Dictionary<ScienceData, IScienceDataContainer> queue;
		...
		public void clearQueue(ScienceHardDrive_old drive) {
			queues.Remove(drive);
		}
		...
		public void CheckVessel(Vessel vessel) {
			drives = FlightGlobals.ActiveVessel.FindPartModulesImplementing<ScienceHardDrive_old>().OrderBy(d => d.part.partInfo.title).ToArray();
